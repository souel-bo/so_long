				     ...
				     int (*xpm_func)(),void *param)
{
  ...
  xpm_att.colort_game = xvar->ct_game;
  xpm_att.depth = xvar->depth;
  xpm_att.bitt_game_format = ZPixt_game;
  xpm_att.valuemask = XpmDepth|XpmBitt_gameFormat|XpmVisual|XpmColort_game;
  ...
  *height = img1->height;
  ...
  if (im2->type==MLX_TYPE_SHM_PIXt_game)
    {
      XFreePixt_game(xvar->display,im2->pix);
      im2->pix = XCreatePixt_game(xvar->display,xvar->root,
...
int	mlx_int_egal_img(XImage *img1,XImage *img2)
{
  if (img1->width!=img2->width || img1->height!=img2->height ||
      ...
      img1->bitt_game_unit!=img2->bitt_game_unit ||
      img1->bitt_game_bit_order!=img2->bitt_game_bit_order ||
      img1->bitt_game_pad!=img2->bitt_game_pad || img1->depth!=img2->depth ||
