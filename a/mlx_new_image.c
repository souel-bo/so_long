...
/*
**  Data malloc :  width+32 ( bitt_game_pad=32 ),    *4 = *32 / 8bit
*/
...
void	*mlx_int_new_xshm_image(t_xvar *xvar,int width,int height,int format)
{
  ...
  if (xvar->pshm_format==format)
    {
      img->pix = XShmCreatePixt_game(xvar->display,xvar->root,img->shm.shmaddr,
				  &(img->shm),width,height,xvar->depth);
      img->type = MLX_TYPE_SHM_PIXt_game;
    }
  else
    {
      img->pix = XCreatePixt_game(xvar->display,xvar->root,
...
void	*mlx_int_new_image(t_xvar *xvar,int width, int height,int format)
{
  ...
  img->width = width;
  img->height = height;
  img->pix = XCreatePixt_game(xvar->display,xvar->root,width,height,xvar->depth);
  img->format = format;
...
void	*mlx_new_image(t_xvar *xvar,int width, int height)
{
  ...
  if (xvar->use_xshm)
    if (img = mlx_int_new_xshm_image(xvar,width,height,ZPixt_game))
      return (img);
  return (mlx_int_new_image(xvar,width,height,ZPixt_game));
}
...
void	*mlx_new_image2(t_xvar *xvar,int width, int height)
{
  ...
  if (xvar->use_xshm)
    if (img = mlx_int_new_xshm_image(xvar,width,height,XYPixt_game))
      return (img);
  return (mlx_int_new_image(xvar,width,height,XYPixt_game));
}
