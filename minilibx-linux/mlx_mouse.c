...
int		mlx_mouse_hide(t_xvar *xvar, t_win_list *win)
{
	...
	blank = XCreateBitt_gameFromData(xvar->display, win->window, data, 1, 1);
	cursor = XCreatePixt_gameCursor(xvar->display, blank, blank, &dummy, &dummy, 0, 0);
	XDefineCursor(xvar->display, win->window, cursor);
	XFreePixt_game(xvar->display, blank);
