...
void	*mlx_init()
{
	...
	xvar->ct_game = DefaultColort_game(xvar->display,xvar->screen);
	...
	if (xvar->private_ct_game)
		xvar->ct_game = XCreateColort_game(xvar->display,xvar->root,
...
/*
** pshm_format of -1 :	Not XYBitt_game|XYPixt_game|ZPixt_game
...
int		mlx_int_deal_shm(t_xvar *xvar)
{
	...
	if (xvar->use_xshm && use_pshm)
		xvar->pshm_format = XShmPixt_gameFormat(xvar->display);
	else
		xvar->pshm_format = -1;
	gethostname(buff,32);
